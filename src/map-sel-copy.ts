...
import { EntityRecArgs, EntityRecArgsIn, MapCopyOptions, copyMapRectsToMap } from 'cc-map-util/map-copy'
import { FsUtil } from './fsutil'
import { MapPoint } from 'cc-map-util/src/pos'

export class BlitzkriegMapUtil {
    copySelMapAreaTo(baseMap: sc.MapModel.Map, selMap: sc.MapModel.Map, sel: Selection,
        offset: MapPoint, filters: EntityRecArgs['filters'], options: MapCopyOptions): sc.MapModel.Map {
    ...
    async copyCurrentSel() {
        ...
        if (! sel) { blitzkrieg.rhudmsg('blitzkrieg', 'Not standing in a sel', 2); return }
